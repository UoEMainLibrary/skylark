...
@section('title', 'Advanced Search - '.config('skylight.fullname'))
    ...
    <div class="searchform" style="display:block">
        ...
        <form action="{{ $collectionUrl('advanced/post') }}" method="post" accept-charset="utf-8">
            ...
            @foreach($searchFields as $label => $field)
                ...
                <p>
                    <label for="{{ $escapedLabel }}" style="width: 100px; float: left; display: block; text-align: right;">{{ $label }}</label>
                    <input type="text" name="{{ $escapedLabel }}" value="" id="{{ $escapedLabel }}" style="margin-left: 15px;" />
                </p>
            @endforeach
            <p>
                <label for="operator" style="width: 100px; float: left; display: block; text-align: right;">Default search operator</label>
                <select name="operator" id="operator" style="margin-left:15px;">
