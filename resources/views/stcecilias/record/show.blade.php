...
@section('title', '"' . $recordTitle . '" — ' . config('skylight.fullname', "St Cecilia's Hall"))

@section('body_class', 'record')

@section('content')
    @php
        // Mirror the legacy stcecilia/views/record.php data preparation.
        ...
        if (! empty($record[$bitstreamField])) {
            ...
            foreach ($byOrder as $bs) {
                ...
                $localUri = url("/stcecilias/record/{$bHandleId}/{$bSeq}/{$bFilename}");
            ...
            foreach ($displayKeys as $key) {
                ...
                foreach ($values as $idx => $val) {
                    ...
                    if (in_array($key, $filters, true)) {
                        ...
                        $href = url('/stcecilias/search/*:*/' . urlencode($key) . ':%22' . $lower . '+%7C%7C%7C+' . $orig . '%22');
                        if (isset($schema[$key])) {
                                                    ...
                                                    <div class="child-meta">
                                                        ...
                                                        <p>
                                                            ...
                                                            @foreach($values as $idx => $val)
                                                                ...
                                                                @else
                                                                    @if(isset($schema[$key]))
                                                                        <span itemprop="{{ $schema[$key] }}" title="Read more about {{ $val }}"><a href="{{ url('/stcecilias/search/%22' . $val . '%22') }}" title="Read more about {{ $val }}">{{ $val }}</a></span>
                                                                    @else
                                                                        {{ $val }}
                                                                    @endif
                                                                @endif
